...
import { fadeIn } from '../utils/motion.js'; // Import animation variants from a utility file

const ExploreCard = ({ id, imgUrl, title, index, active, handleClick }) => (
  <motion.div
    variants={fadeIn('right', 'spring', index * 0.5, 0.75)} // Use fadeIn animation variant with specified settings
  ...
  >
    ...
    {active !== id ? (
      ...
      <div className="absolute bottom-0 p-8 justify-start w-full flex-col bg-[rgba(0,0,0,0.5)] rounded-b-[24px]">
        ...
        <div className="font-normal text-[16px] leaning-[20px] text-white uppercase ">
          Enter the Metaverse
        </div>
        <h2 className="mt[24px] font-semibold sm:[32px] text-[24px] text-white">
          {title}
        </h2>
      </div>
    )}
